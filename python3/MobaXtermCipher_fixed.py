    ...
    def Main(argc: int, argv: list):
        if argc > 2:
            ...
            if conn_hostname != None and conn_username != None and sessionp == None and master_password == None:
                ...
                cipher = MobaXtermCrypto(sys_hostname.encode('utf-8'), sys_username.encode('utf-8'))

                if do_encrypt:
                    print(
                        cipher.EncryptPassword(
                            text.encode('utf-8'), 
                            conn_hostname.encode('utf-8'), 
                            conn_username.encode('utf-8')
                        )
                    )
                else:
                    print(
                ...
                cipher = MobaXtermCrypto(sys_hostname.encode('utf-8'), sys_username.encode('utf-8'), sessionp.encode('utf-8'))
                ...
                cipher = MobaXtermCryptoSafe(master_password.encode('utf-8'))
                if do_encrypt:
                    print(
                        cipher.EncryptPassword(
                            text.encode('utf-8')
